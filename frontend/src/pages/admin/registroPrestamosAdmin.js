                        ...
                        <table className={styles.table}>
                            ...
                            <tbody>
                                {prestamos.map(p => (
                                    <tr
                                        key={p.id_prestamo}
                                        onClick={() => setFilaExpandida(filaExpandida === p.id_prestamo ? null : p.id_prestamo)}
                                        className={styles.expandibleRow}
                                    >
                                        {/* Columna con flecha */}
                                        <td className={styles.arrowCell}>
                                            {filaExpandida === p.id_prestamo ? "▼" : "▶"}
                                        </td>

                                        <td>
                                            {filaExpandida === p.id_prestamo
                                                ? (
                                                    <ul className={styles.listaSinPuntos}>
                                                        {p.llaves.map((l, index) => (
                                                            <li key={index}>{l.nombre_llave}</li>
                                                        ))}
                                                    </ul>
                                                )
                                                : (
                                                    <span>{p.llaves.slice(0, 2).map(l => l.nombre_llave).join(", ")} ...</span>
                                                )
                                            }
                                        </td>
                                        ...
                                        <td>
                                            {filaExpandida === p.id_prestamo
                                                ? (
                                                    <ul className={styles.listaSinPuntos}>
                                                        {p.llaves.map((l, index) => (
                                                            <li key={index}>{l.zona_llave}</li>
                                                        ))}
                                                    </ul>
                                                )
                                                : (
                                                    <span>{p.llaves.slice(0, 2).map(l => l.zona_llave).join(", ")} ...</span>
                                                )
                                            }
                                        </td>
                                            ...
                                            {filaExpandida === p.id_prestamo
                                                ? (
                                                    <div className={styles.estadoLista}>
                                                        {p.llaves.map((l, index) => (
                                                            <div key={index} className={styles.estadoItem}>
                                                                <span className={styles.llaveNombre}>{l.nombre_llave}</span>
                                                                <span className={
                                                                    l.estado === "prestada" ? styles.badgePrestada :
                                                                        l.estado === "anticipada" ? styles.badgeReservada :
                                                                            l.estado === "devuelta" ? styles.badgeDevuelta :
                                                ...
                                                : (
                                                    <div className={styles.estadoItem}>
                                                        <span className={styles.llaveNombre}>{p.llaves[0].nombre_llave}</span>
                                                        <span className={
                                                            p.llaves[0].estado === "prestada" ? styles.badgePrestada :
                                                                p.llaves[0].estado === "anticipada" ? styles.badgeReservada :
                                                                    p.llaves[0].estado === "devuelta" ? styles.badgeDevuelta :
                                                                        styles.badgeDisponible
                                                        }>
                                                            {p.llaves[0].estado}
                                                        </span>
                                                        {p.llaves.length > 2 && <span className={styles.masLlaves}>...</span>}
                                                    </div>
                                                )
                                            }
                                        </td>
                                        {/* Columna de acciones NO se expande */}
